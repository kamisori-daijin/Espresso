...
private func makeDirectSelectRWKVStyleRecurrentWeights(value: Float = 0.01) -> RWKVStyleRecurrentWeights {
...
private func makeDirectSelectRMSNormWeights(value: Float = 1.0) -> TensorBuffer {
...
private func makeDirectSelectClassifierWeights(value: Float = 0.01) -> TensorBuffer {
...
final class RWKVStyleFusedThreeLayerDirectSelectKernelSetTests: XCTestCase {
    func test_compile_specs_expose_single_fused_three_layer_direct_select_kernel() {
        let laneSpatial = 32
        let weights0 = makeDirectSelectRWKVStyleRecurrentWeights(value: 0.01)
        let weights1 = makeDirectSelectRWKVStyleRecurrentWeights(value: 0.02)
        let weights2 = makeDirectSelectRWKVStyleRecurrentWeights(value: 0.03)
        let rmsFinal = makeDirectSelectRMSNormWeights()
        let classifier = makeDirectSelectClassifierWeights()

        let specs = RWKVStyleFusedThreeLayerDirectSelectKernelSet.compileSpecs(
            weights0: weights0,
            weights1: weights1,
            weights2: weights2,
            rmsFinal: rmsFinal,
        ...
        XCTAssertEqual(specs[0].outputSizes, [stateBytes, stateBytes, stateBytes, logitsBytes])
