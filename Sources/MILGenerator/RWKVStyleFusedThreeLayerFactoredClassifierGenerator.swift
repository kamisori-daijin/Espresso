...
public struct RWKVStyleFusedThreeLayerFactoredClassifierGenerator: MILProgramGenerator {
    ...
    public var milText: String {
        ...
        appendLayer(builder: &b, dim: dim, lane: lane, layerIndex: 0, prefix: "l0_",
                    inputX: "x", inputState: "stateIn0", outputX: "l0_xNext", outputState: "stateOut0")
        appendLayer(builder: &b, dim: dim, lane: lane, layerIndex: 1, prefix: "l1_",
                    inputX: "l0_xNext", inputState: "stateIn1", outputX: "l1_xNext", outputState: "stateOut1")
        appendLayer(builder: &b, dim: dim, lane: lane, layerIndex: 2, prefix: "l2_",
                    inputX: "l1_xNext", inputState: "stateIn2", outputX: "xNext", outputState: "stateOut2")

        // Factored RMSNorm + classifier on xNext
        ...
        b.appendLine(
            "        tensor<fp16, [1, \(k), 1, \(lane)]> h_proj = conv(dilations=dl,groups=gr,pad=pd,pad_type=pt,strides=st,weight=Wproj,x=h_xn)[name=string(\"h_proj\")];"
        )
        ...
        b.appendLine(
            "        tensor<fp16, [1, \(vocab), 1, \(lane)]> logits = conv(dilations=dl,groups=gr,pad=pd,pad_type=pt,strides=st,weight=Wexp,x=h_proj)[name=string(\"h_expand\")];"
        )
