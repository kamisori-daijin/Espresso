...
public struct RWKVStyleFusedThreeLayerStepGenerator: MILProgramGenerator {
    ...
    private func appendLayer(
        ...
        b.appendLine("        tensor<fp16, [1,\(dim),1,\(lane)]> \(prefix)xMix = conv(dilations=dl,groups=gr,pad=pd,pad_type=pt,strides=st,weight=\(prefix)Wx,x=\(convInput))[name=string(\"\(prefix)x_mix\")];")
